  ...
  */
  ...
  clickLock = true;
    ...
    if (!killswitch) {
      ...
      charSides[sides.RIGHT] = newerContender(charSides[sides.LEFT]);
  ...
  $("#config").click(function(){
    selection.style.top = '13.5%'; menu.style.top = '2.5%'; menu.textContent = 'BACK';
  });
  
  $("#menu").click(function(){
    landing.style.top = ''; menu.style.top = ''; selection.style.top = '';
...
//////////////////
  ...
  */
  ...
  function clickFunction(side)
  {
    ...
    if (!killswitch) 
    {
        ...
        if(!eligibler(charSides[side]))  //check if the winner needs to be switched out
        { 
    ...
    if (!killswitch) 
    {
      charSides[reverse[side]] = newerContender(charSides[side]);
  ...
  function hover(side)
  {
    ...
    tip.textContent = "Choose " + charSides[side].shorthand + '.';
  }
  ...
  */
  function setNames() {
    ...
    names[sides.LEFT] .textContent = nm1.name;
    names[sides.RIGHT].textContent = nm2.name;
    changeAnim("piccont1","0s"); changeAnim("piccont2","0s");
    picconts[sides.LEFT] .style.backgroundImage = ("url(char/" + nm1.shorthand + ".png)");
    picconts[sides.RIGHT].style.backgroundImage = ("url(char/" + nm2.shorthand + ".png)");
  ...
  */
  ...
  ////////////////
    ...
    function reset(){
      if (chars.length >= 2) {
          ...
          formatResults(11, getCurrentTotalContenders(), 'res-box2', 1.25, FIELD_HEIGHT*RES_BOX_HEIGHT); //TODO, adjust if doing a removal method
      }
    }
    
    function chooseStarters()
    {
      for (j = 0; j < charsdata.length; j++) { expectedSum(charsdata[j], charsdata); } //calculated an expected score for the session
          
      charSides[sides.LEFT] = getContender(getRandomInt(0,3)); //Team RWBY
      if (charSides[sides.LEFT] == null) charSides[sides.LEFT] = randomInCharsData();
      charSides[sides.RIGHT] = newerContender(charSides[sides.LEFT]); // anyone besides the first chosen
    }
    
    function characterBenching()
    {
      charsdata = [];
      bench = [];
      ...
      for (var i = 13; i < 37; ++i)
      {
        ...
        charsdata.splice(contenderIndex, 1);
        bench.push(contender);
      }
    ...
    */
    function isItDone() {
      for (i = 0; i < charsdata.length; i++) { if (eligibler(charsdata[i])) { return false; } }
      ...
      winPic.style.backgroundImage = ("url(char/" + finishedList[0].shorthand + ".png)");
      winnerBox.style.backgroundColor = finishedList[0].color;
      
      for (g = 0; g < finishedList.length; g++){
        var daName = "no" + (1+g);
        var tableItem = document.getElementById(daName);
        tableItem.style.backgroundColor = finishedList[g].color;
        tableItem.textContent = finishedList[g].name;
      }
    ...
    function cGet(charN, isAbove) { //get the above or below array of the id
      return (isAbove ? charN.getAbove() : charN.getBelow());
    }
    function cPush(charN, isAbove, data) { //push data into the above or below
      if (isAbove) { charN.pushAbove(data); } else { charN.pushBelow(data); }
    }
    function exchange(take, to, from) {
      cPush(to, from, take);
      var gets = cGet(take, from)
      for (i = 0; i < gets.length; i++) { 
        var tar = gets[i]; //get target - one of the sender's
    ...
    function getCurrentTotalContenders()
    {
      return charsdata.length;
    }
  ...
  //using a contender and a contendList, find the expected score for the session.
  function expectedSum(contender, cList) {
      var eSum = 0;
      for (l = 0; l < cList.length; l++) {
        var comp = cList[l];
        if (comp != contender) { eSum += expected(contender.getELO(), comp.getELO()); }
      }
      eSum = Math.round(eSum * 100) / 100;
      contender.setSum(eSum);
      return eSum;
  }
  ...
  */
  ...
  function getContender(integer)
  {
    for(var i = 0; i < charsdata.length; ++i)
    {
    var contd = charsdata[i];
    if (contd.getID() == integer) return contd;
    }
    return null;
  }
  ...
  function newerContender(winner)
  {
    ...
    { temp = randomInCharsData(); }; //Anybody besides the person just chosen
    return temp;
  }

  function eligibler(contender) { //PSA, contender takes an object, not an int
    return (contender.getAbove().length + contender.getBelow().length >= getCurrentTotalContenders() - 1 ? false : true);
  }

  function randomInCharsData()
  {
    return charsdata[getRandomInt(0,charsdata.length-1)];
  }

  function eligibleContender() 
  {
    temp = randomInCharsData();
    while(!eligibler(temp)) { randomInCharsData(); }
    return temp;
  }
  ...
  function ele(name) { return document.getElementById(name); }
    ...
    */
    function intSlide(oldv, newv) {
      var max = (getCurrentTotalContenders()*(getCurrentTotalContenders()-1));
      for (t = 0; t < 50; t++) {
